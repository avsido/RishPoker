...
io_client.on("game-start", (currentGame) => {
  ...
  rollOnTheDice(currentGame.dice.opponent, "dice1");
  rollOnTheDice(currentGame.dice.player, "dice2", message, () => {
    setTimeout(() => {
      if (soundOn) openingSound.play();
      window.thisGame = new game(currentGame);
    }, 2500);
  });
});
...
io_client.on("player-played", (currentGame) => {
  console.log(currentGame);
  window.thisGame.update(currentGame);
});
...
io_client.on("player-left", (data) => {
  ...
  window.thisGame.resolveAfterOpponentQuit(matchId);
});
