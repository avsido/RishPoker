function renderMultiplayer() {
  // // displays cards to each player, here the initiator of the game sees player A cards in front of him
  // // player A sees player B cards as opponent cards
  // // if data.contains.drawnCard =  show card
  // // if not > please wait
  ...
  // if (currentGame.cardsLeft == 1) {
  //   cleanElement(divDeck);
  //   imgDrawnCard.style.marginLeft = "35%";
  //   if (currentGame.playerATurn) divDeck.appendChild(imgDrawnCard);
  ...
  for (let i = 0; i < playerCards.length; i++) {
    ...
    for (let j = 0; j < playerCards[i].length; j++) {
      ...
      if (currentGame.cardsLeft == 1 && drawnCard && j == 4) {
        ...
        imgCard.onclick = (ev) => {
          ...
          for (let k = 0; k < playerCards[i].length; k++) {
            ...
            img.src = "images/" + playerCards[i][k].name.toLowerCase() + ".png";
            if (k == 4) {
